function [VrobustOutput, VlowerBound, VupperBound, MnetP, MOutput]=applyAdaptiveBMS(varargin)
...
MOutput         = zeros(NinputSamples, Nnets); % Prediction output by ANN (Cell is necessary if there is more than one output)
MnetP           = zeros(NinputSamples, Nnets); % Posterior value of the prediction by the given ANN

% Compute the Data of interest
for inet=1:Nnets
    if isa(CXann{1,inet},'network')
